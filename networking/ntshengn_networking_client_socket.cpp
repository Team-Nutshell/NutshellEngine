...
NtshEngn::ClientSocket::ClientSocket(NtshEngn::Socket socket, NtshEngn::NetworkType networkType):
...
void NtshEngn::ClientSocket::updateUDP() {
	...
	int receive = recvfrom(static_cast<SOCKET>(m_socket), buffer.data(), NTSHENGN_NETWORKING_BUFFER_SIZE, 0, reinterpret_cast<sockaddr*>(&serverSockaddr), &sockaddrSize);
...
void NtshEngn::ClientSocket::updateTCP() {
	...
	if (m_connectedServer.ipAddress != "") {
		int receive = recv(static_cast<SOCKET>(m_socket), buffer.data(), NTSHENGN_NETWORKING_BUFFER_SIZE, 0);
		if ((receive != 0) && (receive != SOCKET_ERROR)) {
			// Receive data
			if (m_dataReceivedCallback) {
				m_dataReceivedCallback(buffer.data(), receive);
...
void NtshEngn::ClientSocket::connectToServerTCP(const std::string& ipAddress, uint16_t port) {
	...
	if (connect(static_cast<SOCKET>(m_socket), reinterpret_cast<sockaddr*>(&sockaddrIn), static_cast<int>(sizeof(sockaddr_in))) == 0) {
...
void NtshEngn::ClientSocket::sendDataToServerUDP(const void* data, size_t dataSize) {
	...
	serverSockaddr.sin_port = htons(m_connectedServer.port);
	inet_pton(AF_INET, m_connectedServer.ipAddress.c_str(), &serverSockaddr.sin_addr.s_addr);
	if (sendto(static_cast<SOCKET>(m_socket), static_cast<const char*>(data), static_cast<int>(dataSize), 0, reinterpret_cast<sockaddr*>(&serverSockaddr), static_cast<int>(sizeof(sockaddr_in))) == SOCKET_ERROR) {
...
void NtshEngn::ClientSocket::sendDataToServerTCP(const void* data, size_t dataSize) {
	if (send(static_cast<SOCKET>(m_socket), static_cast<const char*>(data), static_cast<int>(dataSize), 0) < 0) {
